...
- [ ] **`server.json` validates against the `$schema` it declares**, not just against a reading of it. Fetch the schema named in the file and run it through a validator rather than eyeballing the shape. Two traps, both of which parse as valid JSON, read as obviously correct, and are rejected on the first publish attempt:
  - **Argument discriminators.** A `packageArguments` entry missing `"type": "named"` fails `NamedArgument`, which requires both `type` and `name`.
  - **String length caps that are not in the obvious place.** `ServerDetail.description` carries `maxLength: 100`, and it lives under `definitions`, not at the top level — so a validator pointed at the root properties reports nothing wrong. Two of three repos audited on one day were at 167 and 118 characters. The `package.json` description is a separate string with no such cap, which is exactly why they drift past it unnoticed. `P1`.
- [ ] **The publish workflow exists at all.** Do not check the workflow's contents before checking that `.github/workflows/` has one. A repo can carry `server.json`, `mcpName`, a `glama.json`, and every packaging artifact while shipping no publish workflow and no CI — the packaging looks complete because every file a reviewer would open is present. Confirm the directory listing, not just the files you went looking for. `P2`.
